...
function TodayMeal({ isLogin }) {
  ...
  useEffect(() => {
    if (isLogin) {
      axios.get(`${process.env.REACT_APP_API_URL}/todaymenu/`).then((resp) => {
  ...
  const removeFood = (idx) => {
    if (modalNum === 1) {
      axios
        .patch(`${process.env.REACT_APP_API_URL}/todaymenu/?when=breakfast`)
        .then(() => console.log('삭제'));
    ...
    if (modalNum === 2) {
      axios
        .patch(`${process.env.REACT_APP_API_URL}/todaymenu/?when=lunch`)
        .then(() => console.log('삭제'));
      lunch.splice(idx, 1);
      setLunch([...lunch]);
    }
    if (modalNum === 3) {
      axios
        .patch(`${process.env.REACT_APP_API_URL}/todaymenu/?when=dinner`)
        .then(() => console.log('삭제'));
      dinner.splice(idx, 1);
      setDinner([...dinner]);
    }
  ...
  const addFoods = (food) => {
    ...
    if (modalNum === 1) {
      ...
      axios
        .post(`${process.env.REACT_APP_API_URL}/todaymenu`, {
          kit_id: food.kit_id,
          when: 'breakfast',
        })
