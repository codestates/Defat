...
function TodayMeal({ isLogin }) {
  ...
  useEffect(() => {
    if (isLogin) {
      axios.get('http://localhost:8080/todaymenu/').then((resp) => {
  ...
  const removeFood = (idx) => {
    if (modalNum === 1) {
      axios
        .patch('http://localhost:8080/todaymenu/?when=breakfast')
        .then(() => console.log('삭제'));
    ...
    if (modalNum === 2) {
      axios
        .patch('http://localhost:8080/todaymenu/?when=lunch')
        .then(() => console.log('삭제'));
      lunch.splice(idx, 1);
      setLunch([...lunch]);
    }
    if (modalNum === 3) {
      axios
        .patch('http://localhost:8080/todaymenu/?when=dinner')
        .then(() => console.log('삭제'));
      dinner.splice(idx, 1);
      setDinner([...dinner]);
    }
  ...
  const addFoods = (food) => {
    ...
    if (modalNum === 1) {
      ...
      axios
        .post('http://localhost:8080/todaymenu', {
          kit_id: food.kit_id,
          when: 'breakfast',
        })
